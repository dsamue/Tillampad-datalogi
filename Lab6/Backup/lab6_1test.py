...
def readMolNumber(q, atomhash):
    x=0    #iom att jag inte sägger x='' och plussar stängar blir det ok att skriva Na00001. kan ju diskuteras om det är rätt.
    while not q.isEmpty():
        if q.peek().isdigit():
            x+=int(q.get())           #ok, nånstans här ska det alltså dyka upp en siffra även om det bara är en etta..
        else: break
